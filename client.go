...
type Client struct {
	cc       codec.Codec
	opt      *Option
	sending  sync.Mutex // 为了保证每个消息的有序发送
	header   codec.Header
	mu       sync.Mutex
	seq      uint64
	pending  map[uint64]*Call // 存储未处理完的请求
	closing  bool             // close是用户主动关闭，shutdown是有错才会关闭
	shutdown bool
}
...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	case <-time.After(opt.ConnectTimeout):
		return nil, fmt.Errorf("rpc client,connect time out expect within %s", opt.ConnectTimeout)
	case result := <-ch:
		return result.client, result.err
	}
}
